...
| **109** | `x11mdl.f:316`'s AIC-reject NOTE -- doubled, because `writln(.., Mt2, Mt2, ..)` is the same unit twice -- and **CB-45** under it: `x11mdl.f:378` writes a CHARACTER through the weekday-header FORMAT and the Fortran runtime KILLS the program, so the oracle's run ends there and this engine adjusted the whole series past it at `OUTCOME: OK`. Also `editor.f:2071`'s 3x15 WARNING, which had to leave `x11_prestage` for `x11_editor_geometry` because its POSITION in the `.err` is observable |
...
| **105** | `Xelong` / `Xhlnln` (`x11regression{eastermeans=}` / `{holidaynonlin=}`) -- both dropped by the parser, and `Xelong` was ALREADY being read from `arima.elong`, the regression spec's copy, at all six x11reg.cpp sites. Oracle: `regression{eastermeans=no}` moves 0 lines of an x11regression run, `x11regression{eastermeans=no}` moves 624. `rgtdhl.f`'s guard is transcribed and its BODY walled, at all eight call sites -- a wall on the OPTION instead refuses runs the oracle completes |
| **106** | board item 8's last four -- `outliermethod=` (`Ladd1x` -> idotlr's `Ladd1`, whose two arms converge on every one of 154 oracle pairs except where addall overruns the regression-effect limit first), `defaultcritical=` (`Cvxtyp`; the reader existed, the observable `x11irrcrtval` did not -- `x12hdr.f:751-764` was unemitted), `x11{taper=}` (`taper.f`, absent from `sautco`), and `almost=`, which the ORACLE cannot parse: CB-43, a dictionary entry padded two blanks past its name. `adrgef.f`'s refusal was two lines of six |
| **104** | `Lttc` (`regression{trendtc=}`) — parsed and DROPPED, so all five `x11pt3`/`x11pt4_etables` call sites passed a hardcoded false. Behind it: `x11pt3.f:926`'s stc2 BUILD condition carries the `Lttc` term and `:1212`'s Part-E PICK does not, and this port had collapsed the two into one `have_stc2` — exact only while the flag could not be set. Same pair as entry 89's `getrev` trend store |
