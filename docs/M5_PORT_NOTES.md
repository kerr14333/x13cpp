...
**The gap was three `writln` lines under a rule the port had already agreed
with.** `parse_spec.cpp` has raised an explicit `forecast{maxlead=}` up to
`max(12,3*Sp)` on a `seats{}` run for months, with a long comment explaining
why -- the forecast decomposition punched 24 rows against the golden's 36
until it did. What it never did was SAY so, and the whole purpose of
`editor.f:396-406` is that the change not be silent. A ported rule whose
diagnostic is unported is a rule the user cannot see fire.

The tuple's comment credited this to `seatop.f`. It is `editor.f:387-406`.
That is now the SEVENTH wrong `.f` attribution in that file's comments, so
this increment re-derived every remaining one mechanically -- join
continuation records, keep the SOURCE line number, grep the message -- rather
than reading them. Four more were wrong (below).

### The number is `itoc`'s, and two quarterly specs are what prove it

`clen` is `CHARACTER*(4)` and `itoc` writes into it at a 1-based position, so
the port pads to 4 and cuts back to `ip2-1` -- no width, no zero padding. It
matters that this is derived rather than pasted: nine of the eleven specs are
monthly and get 36, but `expgs_finite-seats` and `expgs_fixed-airline-seats`
are `period=4`, so their horizon is 12. Hardcoding 36 fails exactly those two.

### The `12` floor cannot be reached by anything in the corpus

`max(12,3*Sp)` equals `3*Sp` for every `Sp>=4`, so on monthly (36) and
quarterly (12) data the floor is algebraically invisible -- dropping it fails
nothing. That is an INERT-BY-CONSTRUCTION result over `Sp>=4` and a CORPUS GAP
below it: `Sp=2` with `seats{}` would give 12 against 6, and no spec has it.
Recorded rather than "fixed", per the entry 95 distinction -- the theorem is
about the arithmetic, not about the program.

### Mutations

| mutation | fails |
|---|---|
| the NOTE never writes | **11** |
| `lblnk` T -> F (the leading blank) | **11** |
| `clen` not cut back to `ip2-1` (4-char padded buffer) | **11** |
| the horizon hardcoded to `36` | **2** -- the two QUARTERLY specs, and only those |
| `max(12,3*Sp)` -> `3*Sp` | 0 -- inert by construction for `Sp>=4` |

The leading-blank row is worth reading beside entry 113's. There it measured
**1** where the golden count said 6, because five of the six specs still
carried the `amdfct` block and were therefore compared through `_collapse`,
which merges blank runs on both sides and cannot see a MISSING blank. Here it
measures the full 11 on the first try -- none of these eleven carries a
remaining unported block, so all eleven are on the strict comparison. **The
