...
c-----------------------------------------------------------------------
      ...
      PARAMETER(NOTYPE=6)
c-----------------------------------------------------------------------
      CHARACTER Otlttl*(*),otltyp*2
      INTEGER Begdat,Begotl,Endotl,itmp,Itype,Nchr,otldat,Sp
...
c     ------------------------------------------------------------------
      IF(Itype.lt.1.or.Itype.gt.NOTYPE)THEN
       CALL errhdr
       WRITE(STDERR,1010)Itype,Otlttl
       WRITE(Mt2,1010)Itype,Otlttl
 1010  FORMAT(/,' ERROR: Invalid outlier type,',i5,', ',a,'.')
       CALL abend
       RETURN
      ELSE
