...
c-----------------------------------------------------------------------
      IF(Bldf.ge.Nefobs)THEN
*       CALL writln('ERROR: Not enough observations to perform automatic
*     &model identification:',STDERR,Mt2,T)
*       CALL writln('       unable to compute Ljung-Box statistic.',
*     &             STDERR,Mt2,F)
*       CALL abend()
*       RETURN
       Bldf = Nefobs / 2
...
c-----------------------------------------------------------------------
      ...
      END DO
      CALL acf(A(i1),Nefobs,Nefobs,smpac,seacf,Bldf,np,Sp,0,T,F)
      Blpct=1D0 - Qpv(Bldf)
      Blq=Qs(Bldf)
c-----------------------------------------------------------------------
