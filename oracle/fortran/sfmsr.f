...
c     ------------------------------------------------------------------
         ...
         ELSE
          ...
          IF(Lsav)WRITE(Nform,1060)i,Ratis
          IF(Ratis.le.2.5D0)THEN
           Lterm=1
           IF(Lprt)WRITE(Mt1,1030)i,Ratis,filter(Lterm)
...
c     ------------------------------------------------------------------
           llda1=llda1-Ny
           IF(Lprt)WRITE(Mt1,1040)i,Ratis
           i=i+1
          END IF
         END IF
        END DO
...
c     ------------------------------------------------------------------
      RETURN
c     ------------------------------------------------------------------
 1010 FORMAT('1       Final Seasonal Filter Selection via GLOBAL MSR',/)
 1020 FORMAT('  ***  Not enough data to continue, 3x5 seasonal filter ',
     &       'selected  ***')
 1030 FORMAT(' Pass No. ',i2,':  Global MSR = ',f6.2,', ',a3,
     &       ' seasonal filter selected.')
 1040 FORMAT(' Pass No. ',i2,':  Global MSR = ',f6.2)
 1050 FORMAT('sfmsr: ',a3)
 1060 FORMAT('autosf.msr',i2.2,': ',f6.2)
c     ------------------------------------------------------------------
      END
