C     Last change: LS Oct.24,2023-save d7trendma in .udg file
C     previous change: LS Nov. 1 2022- saved .b1 file
         ...
         IF(i.lt.mfda.or.i.gt.mlda)THEN
          IF(dpeq(Sts(i),ZERO))THEN
           IF(.not.Lhiddn)THEN
            ...
            CALL errhdr
            WRITE(Mt2,1020)
 1020       FORMAT(/,' ERROR: Initial seasonal factor is equal to ',
     &               'zero - cannot continue with',
     &             /,'        pseudo-additive seasonal adjustment.',/)
           END IF
           CALL abend
           RETURN
          ELSE
...
c-----------------------------------------------------------------------
c     save d7trendma in .udg file
       If (Ktcopt.eq.0.and.Kpart.eq.4.and.Lsumm.gt.0.and.gudrun) then
         WRITE(Nform,1750)Nterm
1750     FORMAT('d7trendma: ',i3)
       end if
       fext=LXEPTN+Kpart-2
       IF(Prttab(fext))THEN
        IF(oktrn)THEN
