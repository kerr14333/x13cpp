...
c                (301) 763-3957
c-----------------------------------------------------------------------
c   Revised by Brian C. Monsell for X-12, X-13A-S
c-----------------------------------------------------------------------
c     INCLUDE 'stdio.i'
c-----------------------------------------------------------------------
c     call fclose(ALL)
...
c-----------------------------------------------------------------------
      IF(Issap.eq.2.and.(Lsumm.gt.0.or.Svltab(LSLPCT)))THEN
       ...
       IF(Svltab(LSLPCT))WRITE(Ng,1035)
     ...
     &        Lsumm.gt.0))THEN
       ...
       END IF
       IF(istrue(Svltab,LSLASA,LSLASP))WRITE(Ng,1045)
      END IF
...
c-----------------------------------------------------------------------
 ...
 1035 FORMAT(/,' Sliding spans analysis failed : check error file.')
 1045 FORMAT(/,' History analysis failed : check error file.')
