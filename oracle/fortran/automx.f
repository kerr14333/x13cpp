...
c     ------------------------------------------------------------------
      ...
      IF(Savtab(LOTLFT).or.Savtab(LOTLIT))THEN
       CALL writln('NOTE: Tables associated with the outlier spec cannot
     & be saved during',fhnote,Mt2,T)
       CALL writln('      automatic model selection.',fhnote,Mt2,F)
      END IF
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      ...
      IF(havfil)THEN
       ...
       IF(Lfatal)RETURN
       IF(.not.inptok)THEN
        CALL writln('ERROR: Must have user supplied models stored in '//
     &             Autofl(1:nblank(Autofl))//'.',STDERR,Mt2,T)
        CALL abend
        RETURN
       END IF
      END IF
...
c-----------------------------------------------------------------------
         IF(nummdl.eq.0)THEN
          WRITE(STDERR,1050)Autofl(1:nblank(Autofl))
          WRITE(Mt2,1050)Autofl(1:nblank(Autofl))
          CALL abend
          RETURN
         END IF
        END IF
...
c-----------------------------------------------------------------------
        IF(.not.inptok)THEN
         IF(havfil)THEN
          IF(.not.Lquiet)WRITE(STDERR,1040)Autofl(1:nblank(Autofl))
          WRITE(Mt2,1040)Autofl(1:nblank(Autofl))
         ELSE
          IF(.not.Lquiet)WRITE(STDERR,1040)Autofl(1:nblank(Autofl))
          WRITE(Mt2,1040)Autofl(1:nblank(Autofl))
         END IF
...
c-----------------------------------------------------------------------
        IF(.not.Hvmdl.and.Hvstar.eq.0)THEN
         IF(nerr.gt.0)THEN
          IF(.not.Lquiet)WRITE(STDERR,1120)Cursrs(1:nblank(Cursrs))
          WRITE(Mt1,1120)Cursrs(1:nblank(Cursrs))
         END IF
         IF(Prttab(LAXMCH))WRITE(Mt1,1060)
         Bstdsn(1:4)='none'
         Nbstds=4
         RETURN
        END IF
...
c-----------------------------------------------------------------------
        ...
        IF(havfil)THEN
         ...
         IF(Lfatal)THEN
          WRITE(STDERR,1070)Autofl(1:nblank(Autofl))
          WRITE(Mt2,1070)Autofl(1:nblank(Autofl))
          RETURN
         END IF
        ELSE
...
c-----------------------------------------------------------------------
        IF(havfil.and.Nxtktp.eq.STAR)THEN
         IF(Hvstar.gt.0)THEN
          IF(.not.Lquiet)WRITE(STDERR,1071)Autofl(1:nblank(Autofl))
          WRITE(Mt2,1071)Autofl(1:nblank(Autofl))
         ELSE
          Hvstar=1
         END IF
...
c-----------------------------------------------------------------------
     ...
     &              F,F,0,Lhiddn)
         IF(Lfatal)RETURN
         IF(lester)THEN
          CALL writln('ERROR: An model estimation error has occurred dur
     &ing the AIC testing of',STDERR,Mt2,T)
          CALL writln('       trading day regressor(s).  The error messa
     &ge appears below.',STDERR,Mt2,F)
        ...
        IF((.not.lester).and.Lomtst.gt.0)THEN
         ...
         IF(lester)THEN
          CALL writln('ERROR: An model estimation error has occurred dur
     &ing the AIC testing of',STDERR,Mt2,T)
          CALL writln('       lom/loq/lpyear regressor(s).  The error me
     &ssage appears below.',STDERR,Mt2,F)
          CALL prterr(nefobs,T)
          IF(Id1st)THEN
           IF(Prttab(LAXMCH))WRITE(Mt1,1060)
           Bstdsn(1:4)='none'
        ...
        IF((.not.lester).and.Leastr)THEN
         ...
         IF(lester)THEN
          CALL writln('ERROR: An model estimation error has occurred dur
     &ing the AIC testing of',STDERR,Mt2,T)
          CALL writln('       an Easter regressor.  The error message ap
     &pears below.',STDERR,Mt2,F)
          CALL prterr(nefobs,T)
          IF(Id1st)THEN
           IF(Prttab(LAXMCH))WRITE(Mt1,1060)
           Bstdsn(1:4)='none'
        ...
        IF((.not.lester).and.(Luser.and.Ncusrx.gt.0))THEN
         ...
         IF(lester)THEN
          CALL writln('ERROR: An model estimation error has occurred dur
     &ing the AIC testing of',STDERR,Mt2,T)
          CALL writln('       user defined regressor(s).  The error mess
     &age appears below.',STDERR,Mt2,F)
        ...
        IF(.not.lester.and.(Ch2tst.and.Nguhl.gt.0))THEN
         ...
         IF(lester)THEN
          CALL writln('ERROR: An model estimation error has occurred dur
     &ing the chi square testing',STDERR,Mt2,T)
          CALL writln('       of user defined holiday regressor(s).  The
     & error message appears below.',STDERR,Mt2,F)
          CALL prterr(nefobs,T)
          IF(Id1st)THEN
           IF(Prttab(LAXMCH))WRITE(Mt1,1060)
           Bstdsn(1:4)='none'
     ...
     &               F,F,F,F)
         IF((.not.Lfatal).and.(.not.Convrg))THEN
          IF(Id1st)THEN
           WRITE(Mt1,1056)
           IF(havfil)THEN
            WRITE(Mt1,1057)Autofl(1:nblank(Autofl))
           ELSE
            WRITE(Mt1,1058)
           END IF
           WRITE(Mt1,1059)PRGNAM
           IF(Prttab(LAXMCH))WRITE(Mt1,1060)
           Bstdsn(1:4)='none'
           Nbstds=4
           Hvmdl=F
...
c-----------------------------------------------------------------------
        IF(Armaer.lt.0.or.Armaer.gt.1)THEN
         nerr=nerr+1
         WRITE(Mt2,1080)Mdldsn(1:Nmddcr)
...
c-----------------------------------------------------------------------
     ...
     &      Armaer.eq.POBFN0.or.Armaer.eq.PACSER.or.Armaer.lt.0)THEN
          ...
          IF(Hvstar.eq.1)THEN
           Hvstar=0
           IF(.not.Lquiet)WRITE(STDERR,1072)
           WRITE(Mt2,1072)
          END IF
         END IF
         Armaer=0
...
c-----------------------------------------------------------------------
     ...
     &              F,F,0,Lhiddn)
         IF(Lfatal)RETURN
         IF(lester)THEN
          CALL writln('ERROR: A model estimation error has occurred duri
     &ng the AIC testing of',STDERR,Mt2,T)
          CALL writln('       trading day regressor(s).  The error messa
     &ge appears below.',STDERR,Mt2,F)
          CALL prterr(nefobs,T)
          IF(Prttab(LAXMCH))WRITE(Mt1,1060)
...
*        CALL idotlr(Ltstao,Ltstls,Ltsttc,Ltstso,Ladd1,Critvl,Cvrduc,
     ...
     &              F,F,F,F)
        IF((.not.Lfatal).and.(.not.Convrg))THEN
         WRITE(Mt1,1050)Autofl(1:nblank(Autofl))
         IF(Prttab(LAXMCH))WRITE(Mt1,1060)
         Bstdsn(1:4)='none'
         Nbstds=4
         Hvmdl=F
...
c-----------------------------------------------------------------------
      ELSE IF(Prttab(LAXMCH))THEN
       WRITE(Mt1,1090)Bstdsn(1:Nbstds)
      END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
       IF(Prttab(LAXHDR))WRITE(Mt1,1100)
c-----------------------------------------------------------------------
       IF(Prttab(LAXMDL))THEN
...
c-----------------------------------------------------------------------
      ...
      END IF
c-----------------------------------------------------------------------
 1010 FORMAT(/,' Autoregressive Integrated Moving Average (ARIMA) ',
     &       'extrapolation program',/,' ARIMA extrapolation',
     &       ' model (forecast)',/)
 1011 FORMAT(5X,'Autoregressive Integrated Moving Average (ARIMA) ',
     &       'extrapolation program',/,'       ARIMA extrapolation',
     &       ' model (forecast)',/)
 1020 FORMAT(' Model selected:      First model that meets ',
     &       'acceptance criteria.')
 1021 FORMAT(/,5x,'Model selected: First model that meets acceptance',
     &       ' criteria.')
 1030 FORMAT(' Model selected:      Model with lowest average ',
     &       'forecast error that',/,' meets acceptance criteria.')
 1031 FORMAT(/,5x,'Model selected: Model with lowest average ',
     &       'forecast error that meets',/,21x,'acceptance criteria.')
 1040 FORMAT(/,' WARNING: Unable to process at least one of the ARIMA',
     &         ' models stored in ',/,'          ',a,'.',/,
     &       '          Check contents of this file and try again.')
 1041 FORMAT(/,' WARNING: Unable to generate at least one of the ',
     &         'default ARIMA models.')
 1050 FORMAT(/,' ERROR: No ARIMA models stored in ',a,'.',
 ...
 1056 FORMAT('        Rerun program trying one of the following:',/,
     ...
     &       'value of maxiter).')
 1057 FORMAT('          (2) Remove model from automatic model ',
     &       'file',1x,a,'.')
 1058 FORMAT('          (2) Use an automatic model file to specify ',
     &       'other models.')
 1059 FORMAT('        See ',a,' of the ',a,' ',a,' for more',/,
     &            ' discussion.')
 1060 FORMAT(//,15x,'None of the models were chosen.',/)
 1070 FORMAT(/,' ERROR: Unable to read automatic model for the ',
     &         'reason(s) given above.',/,
     &         '        Check the models stored in ',a,'.')
 1071 FORMAT(/,' NOTE: Default model already specified.',/,
     &         '       Check the model file ',a,'.')
 1072 FORMAT(/,' WARNING: Estimation error encountered for default ',
     &         'model.',/,'          Default model will not be used.')
 1080 FORMAT(' Estimation errors associated with the model: ',a)
 1090 FORMAT(//,15x,'The model chosen is ',a,/)
 1091 FORMAT(15x,'A default model specified by the user, ',a,',',/,
     &       15x,'will be used to generate regARIMA preadjustment ',
     &           'factors')
 1100 FORMAT(5X,'Autoregressive Integrated Moving Average (ARIMA) ',
     &       'extrapolation program',/,7x,'ARIMA extrapolation model ',
     &       '(backcast)',/)
 1110 FORMAT(//,15x,'This model was not chosen for backcasting.',/)
 1120 FORMAT(/,' WARNING: Estimation errors occured during the ',
     &         'automatic model ',
     &       /,'          selection procedure.  For more details, ',
     &         'check error file ',
     &       /,'          ',a,'.err ')
 1130 FORMAT(' WARNING: The seasonal MA coefficient(s) for at least',
     &       ' one of the models',/,
     &       '          tested above have a sum exceeding ',f6.3,'.',
     &    //,'          Examine whether a differencing can be ',
     &       'eliminated from the',/,
     &       '          regARIMA model in favor of a trend constant ',
     &       'in the regression',/,
     &       '          spec, or whether a seasonal differencing ',
     &       'should be replaced',/,
     &       '          by the use of fixed seasonal effects in the ',
     &       'regression spec.')
 1140 FORMAT(a,': ',a)
 1150 FORMAT(a,': ',f12.6)
c-----------------------------------------------------------------------
