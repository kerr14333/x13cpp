...
c-----------------------------------------------------------------------
     ...
     &              Tfmt,Colwid,nc,1,1,0,0,0,0,F)
       ELSE IF(Ipby.eq.2)THEN
        CALL wrttbl(trv,j,'XXXXX',nc,2,Mt1,Tfmt,Colwid,nc,1,1,0,0,0,0,F)
       ELSE IF(Ipby.eq.3)THEN
        CALL wrttbl(trv,j,hvec(i),nc,2,Mt1,Tfmt,Colwid,nc,1,1,0,0,0,0,F)
       ELSE
        CALL wrttbl(trv,j,'     ',nc,2,Mt1,Tfmt,Colwid,nc,1,1,0,0,0,0,F)
       END IF
      END DO
c-----------------------------------------------------------------------
