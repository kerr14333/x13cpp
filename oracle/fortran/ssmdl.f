...
c-----------------------------------------------------------------------
     ...
     &         Rgvrtp(begcol).eq.PRGTSO.or.Rgvrtp(begcol).eq.PRGTTL)THEN
...
c-----------------------------------------------------------------------
      IF(ngr1tl.gt.0)THEN
       CALL writln('NOTE: The following change of regime regression vari
     &ables are not',Mt1,Mt2,T)
       CALL writln('      defined for at least one year of one of the sp
     &ans of the',Mt1,Mt2,F)
       CALL writln('      sliding spans analysis:',Mt1,Mt2,F)
       DO igrp=1,ngr1tl
     ...
     &                Mt1,Mt2,T)
      ...
      IF(ngr2tl.gt.0)THEN
       CALL writln('NOTE: The following change of regime regression vari
     &ables could cause',Mt1,Mt2,T)
       CALL writln('      singularity problems in the regression matrix 
     &for at least one',Mt1,Mt2,F)
...
c-----------------------------------------------------------------------
       ...
       IF(nbeg.gt.0.or.nend.gt.0)THEN
        ...
        CALL writln('NOTE: Since a model span is used, model parameters 
     &will be held fixed.',fhnote,Mt2,T)
       END IF
      END IF
c-----------------------------------------------------------------------
