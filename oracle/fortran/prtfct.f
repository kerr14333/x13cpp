...
c     ------------------------------------------------------------------
      ...
      IF(Lfatal)RETURN
      IF(.not.Lhiddn.and.istrue(Prttab,LFORTS,LFOROS))WRITE(Mt1,1010)
     &   str(1:ndtchr),Nfcst
 1010 FORMAT(//,' FORECASTING',/,'  Origin',a10,/,'  Number',i10)
c-----------------------------------------------------------------------
     ...
     &   Svdiag)THEN
       IF(Prttab(LFORTS))THEN
        IF((lpria.or.lprix).and.ltrns)THEN
         WRITE(Mt1,1020)
     &'Forecasts and Standard Errors of the Prior Adjusted and Transform
     &ed Data'
 1020    FORMAT(/,'  ',a)
        ELSE IF(ltrns)THEN
         WRITE(Mt1,1020)
     ...
     &        'Forecasts and Standard Errors of the Prior Adjusted Data'
        ELSE
         WRITE(Mt1,1020)'Forecasts and Standard Errors'
        END IF
       END IF
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
        DO i=1,Nfcst
         ...
         IF(Lfatal)RETURN
c     ------------------------------------------------------------------
         IF(Prttab(LFORTS))WRITE(Mt1,fmt)str(1:ndtchr),fcst(i),fcstse(i)
c     ------------------------------------------------------------------
         IF(Savtab(LFORTS).OR.(Lgraf.and.ltrns))THEN
          npos=1
...
c     ------------------------------------------------------------------
        DO i=1,nobsf
         ...
         IF(Lfatal)RETURN
c     ------------------------------------------------------------------
         IF(Prttab(LFORTS))WRITE(Mt1,fmt)str(1:ndtchr),trnsrs(i),fcst(i)
     &                           ,fcterr(i),fcstse(i),tval(i)
...
c     ------------------------------------------------------------------
         DO i=nobsf+1,Nfcst
          ...
          CALL wrtdat(idate,Sp,str,ndtchr)
          IF(Lfatal)RETURN
          IF(Prttab(LFORTS))WRITE(Mt1,fmt)str(1:ndtchr),fcst(i),
     &                                    fcstse(i)
c     ------------------------------------------------------------------
          IF(Savtab(LFORTS).OR.(Lgraf.and.ltrns))THEN
           npos=1
...
c     ------------------------------------------------------------------
       DO i=1,Nfcst
        ...
        IF(Prttab(LFORVR))WRITE(Mt1,fmt)str(1:ndtchr),regvar,stctvr,
     &                                  totvar,pctrgv
c     ------------------------------------------------------------------
        IF(Savtab(LFORVR))THEN
         npos=1
...
c     ------------------------------------------------------------------
         IF(Prttab(LFOROS).or.Savtab(LFOROS).or.Lgraf)THEN
          IF(.not.Lquiet)WRITE(STDERR,1120)
          CALL errhdr
          WRITE(Mt2,1120)
         END IF
 1120    FORMAT(/,
...
c     ------------------------------------------------------------------
       IF(Prttab(LFOROS))THEN
        WRITE(Mt1,1020)ttlfct(1:nttlcr)
        IF(ntl2cr.gt.0)THEN
         WRITE(Mt1,1130)ttlfc2(1:ntl2cr)
 1130    FORMAT('  ',a)
        END IF
        IF(Lognrm.and.dpeq(Lam,ZERO))
     &     WRITE(Mt1,1130)'with LogNormal correction'
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
       DO i=1,Nfcst
        ...
        CALL wrtdat(idate,Sp,str,ndtchr)
        IF(Lfatal)RETURN
        IF(Prttab(LFOROS))WRITE(Mt1,fmt)str(1:ndtchr),lwrci(i),untfct(i)
     &                                  ,uprci(i)
