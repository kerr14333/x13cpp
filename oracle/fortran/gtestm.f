...
c-----------------------------------------------------------------------
   50   CALL gtdcvc(LPAREN,T,1,ESTDIC,estptr,PEST,
     &              'Choices are fixed or estimated',ivec,nelt,argok,
     &              Inptok)
        IF(Lfatal)RETURN
c-----------------------------------------------------------------------
        IF(nelt.gt.0)THEN
...
c-----------------------------------------------------------------------
  100   IF(Hvarma.or.Havreg)THEN
         CALL inpter(PERROR,Errpos,'Cannot specify a model file when a r
     &egARIMA model is specified in')
         CALL writln('the arima and/or regression specs.',Mt2,STDERR,F)
         Inptok=F
        END IF
  ...
  115   CALL gtdpvc(LPAREN,T,1,dvec,nelt,argok,Inptok)
        ...
        IF(nelt.gt.0)THEN
         ...
         IF(Stepln.lt.0D0)THEN
          CALL inpter(PERROR,Errpos,'Step size of numerical derivatives 
...
c-----------------------------------------------------------------------
  116   CALL gtdpvc(LPAREN,T,1,dvec,nelt,argok,Inptok)
        ...
        IF(nelt.gt.0)THEN
         Eick=dvec(1)
         IF(Eick.le.0D0)THEN
          CALL inpter(PERROR,Errpos,'Penalty term for EIC cannot be less
