...
c-----------------------------------------------------------------------
     ...
     &              Priadj,Reglom,Nrxy,Begxy,Frstry,T,Elong)
        ...
        IF(Lfatal)RETURN
        IF(Lprt)THEN
         WRITE(Mt1,1010)'  Model changed to '
...
c-----------------------------------------------------------------------
        ...
        END IF
        CALL mkmdsn(Ipr,Idr,Iqr,Ips,Ids,Iqs,Bstdsn,Nbstds)
        IF(Lprtlb)WRITE(Mt1,1060)Bstdsn(1:Nbstds)
        IF(Lfatal)RETURN
        CALL mdlint()
...
c-----------------------------------------------------------------------
         IF(Ipr.lt.3)THEN
          CALL mkmdsn(Ipr,Idr,Iqr,Ips,Ids,Iqs,Bstdsn,Nbstds)
          IF(Lprtlb)WRITE(Mt1,1070)Bstdsn(1:Nbstds)
          IF(Lfatal)RETURN
         END IF
        END IF
...
c-----------------------------------------------------------------------
 ...
 1030 FORMAT('    Critical Value for ',a,' outlier id changed to:',1x,
     &       F12.3)
 1040 FORMAT(/,'  Confidence coefficient for Ljung-Box Q at lag ',i3,
     &         ' = ',f10.4,','
     &       /,'  which is greater than the acceptance limit, ',f10.4,
     &         '.')
 1050 FORMAT(/,'  Automatic outlier identification will be redone.',/)
 1060 FORMAT(/,'  Model changed to ',a,
     &         ' due to unacceptable Ljung-Box Q statistics',
     &       /,'  for previously identified models.')
 1070 FORMAT(/,'  Model changed to ',a,' due to estimation errors.')
      END
