...
c     ------------------------------------------------------------------
      IF(nttl.gt.2)THEN
       WRITE(Mt1,1010)
 1010  FORMAT(' Combined Prior Adjustment Factors')
       WRITE(Mt1,1020)ttl(1:nttl)
 1020  FORMAT(a)
       IF(Reglom.eq.2)THEN
        WRITE(Mt1,1020)'  Trading Day variables adjusted too'
       ELSE IF(Reglom.eq.3)THEN
        WRITE(Mt1,1020)'  All regression variables adjusted too'
       END IF
      END IF
c     ------------------------------------------------------------------
      RETURN
