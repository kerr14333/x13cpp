C     Last change:  BCM  16 Feb 1999    3:50 pm
      SUBROUTINE prtrev(Fin,Cnc,Revspn,Tbltyp,Nptr,Ntargt,Vtargt,Lsumm,
     &                  Lgraf,Lr1y2y)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &          hdrttl*(PCOLRV*PC1),hd2ttl*(PCOLRV*PC1),tfmt2*(110),
     &          outstr*(6+(23*PC1)),revlbl*(20),laglbl*(5),brklbl*(7)
     ...
     &        idate,rdbdat,ntbttl,ncol,i2,i0,npos,iper,hdrptr,hd2ptr,
     ...
     &               hdrptr,nhdrtl)
        ELSE
         IF(Cnctar.or.i2.eq.0)THEN
          CALL insstr('Conc -',nhdr,PC1,hdrttl,hdrptr,nhdrtl)
         ELSE
          ipos=1
...
c-----------------------------------------------------------------------
      IF(Svltab(LSLASA+Tbltyp-1))THEN
       IF(Tbltyp.eq.1)THEN
        nlbl=13
        revlbl(1:nlbl)='Seasonal Adj.'
       ELSE IF(Tbltyp.eq.2)THEN
        nlbl=15
        revlbl(1:nlbl)='Changes in Adj.'
       ELSE IF(Tbltyp.eq.3)THEN
        nlbl=18
        revlbl(1:nlbl)='Ind. Seasonal Adj.'
       ...
       END IF
       WRITE(Ng,1050)revlbl(1:nlbl),aartot(0,1)
c-----------------------------------------------------------------------
c       IF(ncol.gt.0)THEN
c        DO i=1,ncol
...
c-----------------------------------------------------------------------
       ...
       IF(Tbltyp.eq.2.or.Tbltyp.eq.5)THEN
        WRITE(tfmt1,1021)tbw,2,tbw
        WRITE(tfmt2,1031)tbw,2,tbw
       ELSE
        WRITE(tfmt1,1021)tbw,1,tbw
        WRITE(tfmt2,1031)tbw,1,tbw
       END IF
 1021  FORMAT('(1x,a5,1x,2(1x,f',i2,'.',i1,'),1x,f',i2,'.2)')
 1031  FORMAT('(2x,i4,1x,2(1x,f',i2,'.',i1,'),1x,f',i2,'.2)')
c-----------------------------------------------------------------------
       DO i=0,Ntargt
        i0=0
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &               nhdrtl)
        ...
        IF(Lfatal)RETURN
c-----------------------------------------------------------------------
        CALL prtshd(tblttl(1:ntbttl),Revspn,Ny,rnum,T)
        IF(Lfatal)RETURN
c-----------------------------------------------------------------------
        DO j=1,rnum
         IF(Cnctar.or.i.eq.0)THEN
...
c-----------------------------------------------------------------------
        ...
        DO WHILE (k.le.end2)
         ...
         ipos=1
         CALL itoc(j,cobs,ipos)
         CALL prrvob(tmp,2,k-Begrev+1,k2-Begrev+1,1,1,cobs,ipos-1,
     &               hdrttl,hdrptr,nhdrtl,hd2ttl,hd2ptr,0,i0,
     &               tfmt1(1:npos),k3,2,3,tbw,tblttl(1:ntbttl),F)
         IF(Lfatal)RETURN
         IF(k.eq.Begrev)k3=1
         k=k2+1
         k2=k2+Ny
