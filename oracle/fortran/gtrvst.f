C     Last change: Nov. 13, 2023 roll back to Build 58 to set the
C     estimate defaut to adj
C     previous change:  Mar.2021 allow oltwin equals to 0
C     Last change:  BCM  23 Mar 2005    3:07 pm
...
c     ------------------------------------------------------------------
   10   CALL gtdcvc(LPAREN,F,PRVEST,ESTDIC,estptr,PRVEST,'Choices of est
     &imates are sadj, seasonal, sadjchng, trend, trendchng,',
...
c     ------------------------------------------------------------------
        IF(argok)THEN
         ...
         END DO
        ELSE
         CALL writln('        aic, fcst, arma, and td.',STDERR,Mt2,F)
        END IF
        GO TO 210
c-----------------------------------------------------------------------
c     sadjlags variable
...
c     ------------------------------------------------------------------
         DO i=1,Nfctlg
          IF(Rfctlg(i).le.0)THEN
           CALL inpter(PERROR,Lstpos,
     &                 'Entries for fstep must be greater than zero.')
           Inptok=F
          END IF
c     ------------------------------------------------------------------
          IF(Rfctlg(i).gt.PFCST)THEN
           CALL inpter(PERROR,Lstpos,'Entries for fstep cannot exceed th
     &e maximum value specified for maxlead.')
           Inptok=F
          END IF
         END DO
c     ------------------------------------------------------------------
     ...
     &              ivec2,nelt,argok,Inptok)
        ...
        IF(argok)THEN
         ...
         IF(nelt.eq.2.and.Otlrev.eq.1)THEN
          CALL inpter(PERROR,Errpos,'Cannot specify both remove and keep
     & for the outlier argument.')
          Inptok=F
         END IF
        END IF
        GO TO 210
...
c-----------------------------------------------------------------------
  110   CALL getivc(LPAREN,T,1,ivec,nelt,argok,Inptok)
        ...
        IF(argok.and.Otlwin.lt.0)THEN
         CALL inpter(PERROR,Errpos,'Value of outlierwin must be an integ
     &er greater than or equal to zero.')
         Inptok=F
        END IF
        GO TO 210
c     ------------------------------------------------------------------
...
c-----------------------------------------------------------------------
       IF(Irev.eq.0)Irev=1
c-----------------------------------------------------------------------
c     roll back to Build 58 to set the estimate defaut to adj
c-----------------------------------------------------------------------
       IF(.not.Lrvsa.and.Ntarsa.gt.0)Lrvsa=T
       IF(.not.Lrvtrn.and.Ntartr.gt.0)Lrvtrn=T
       IF((.not.Lrvsa).and.(.not.Lrvsf).and.(.not.Lrvch).and.
     &    (.not.Lrvtrn).and.(.not.Lrvaic).and.(.not.Lrvfct).and.
     &    (.not.Lrvtch).and.(.not.Lrvarma).and.(.not.Lrvtdrg))Lrvsa=T
...
c-----------------------------------------------------------------------
         IF(.not.Lrvsa)THEN
          ...
          WRITE(STDERR,1010)
          WRITE(Mt2,1010)
 1010     FORMAT(/,' WARNING: Need to specify revisons history for  ',
     &            'seasonal adjustments in all ',/,
     &            '           components of a composite adjustment to ',
     &            'get a revisions history of the',/,
...
c-----------------------------------------------------------------------
     ...
     &                  Indrvs(MO).eq.Rvstrt(MO)))THEN
           ...
           WRITE(STDERR,1020)
           WRITE(Mt2,1020)
 1020      FORMAT(/,' WARNING: Starting date of revisons history ',
...
c-----------------------------------------------------------------------
         ELSE IF(Rvstrt(YR).eq.0.and.Indrev.gt.0)THEN
          ...
          WRITE(STDERR,1030)
          WRITE(Mt2,1030)
 1030     FORMAT(/,' WARNING: Starting date of revisons history ',
     ...
     &             '          indirect seasonally adjusted series.')
        ...
        IF(lprt2)THEN
         WRITE(STDERR,1040)
         WRITE(Mt2,1040)
 1040    FORMAT(/,'          Edit all input specification files ',
