      SUBROUTINE wrttb2(Tmp,Ctmp,Jyr,Tyrly,L,Kdec,Mt1,Tblfmt,Tblwid,
     &                  Tblcol,Disp1,Disp2,Disp3,Nb,Nb1,Ipow,Disp4,
     &                  Lastcl)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
       tw2=Tblwid-1
       IF((Disp2-Disp4).gt.0)THEN
        IF(tw2.le.9)THEN
         WRITE(dfmt,1010)Disp2-Disp4,tw2,Kdec
 1010    FORMAT('(',i1,'x,f',i1,'.',i1,',a)')
        ELSE
         WRITE(dfmt,1020)Disp2-Disp4,tw2,Kdec
 1020    FORMAT('(',i1,'x,f',i2,'.',i1,',a)')
        END IF
       ELSE
        IF(tw2.le.9)THEN
         WRITE(dfmt,1011)tw2,Kdec
 1011    FORMAT('(f',i1,'.',i1,',a)')
        ELSE
         WRITE(dfmt,1021)tw2,Kdec
 1021    FORMAT('(f',i2,'.',i1,',a)')
        END IF
       END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
         ...
         IF(dpeq(Tmp(j),DNOTST).or.Tmp(j).ge.BIG)THEN
          ...
          END IF
         ELSE
          WRITE(xlin((ipos+1):ipos2),dfmt)postmp(j),Ctmp(j)
         END IF
         ipos=ipos2
        END DO
...
c-----------------------------------------------------------------------
       IF(Tyrly.eq.'XXXXX')THEN
        IF(Lastcl)THEN
         WRITE(Mt1,Tblfmt)Jyr,(postmp(j),ctmp(j),j=1,L-1),postmp(L)
        ELSE
         WRITE(Mt1,Tblfmt)Jyr,(postmp(j),ctmp(j),j=1,L)
        END IF
       ELSE
        WRITE(Mt1,Tblfmt)Tyrly,(postmp(j),ctmp(j),j=1,L)
       END IF
      END IF
c-----------------------------------------------------------------------
      RETURN
