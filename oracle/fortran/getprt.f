...
c-----------------------------------------------------------------------
     ...
     &               'Print or level argument is not defined.')
         CALL writln('        Check the available table names and levels
     & for this spec.',STDERR,Mt2,F)
         CALL lex()
         Inptok=F
c     ------------------------------------------------------------------
        ELSE
...
c     -----------------------------------------------------------------
       DO WHILE (T)
        IF(Nxtktp.eq.EOF)THEN
         CALL inpter(PERROR,Lstpos,'Unexpected EOF')
         Inptok=F
         GO TO 20
...
c-----------------------------------------------------------------------
     ...
     &                 argok)
          ...
          IF(tblidx.eq.0)THEN
           CALL inpter(PERROR,Lstpos,
     &                 'Print or level argument is not defined.')
           CALL writln('        Check the available table names and leve
     &ls for this spec.',STDERR,Mt2,F)
           CALL lex()
           Inptok=F
c     -----------------------------------------------------------------
          ELSE
