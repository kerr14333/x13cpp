...
c-----------------------------------------------------------------------
     ...
     &        endcol,jcol,itmp,newreg,ireg,ipos
...
c     ------------------------------------------------------------------
     ...
     &      Vartyp.eq.PRSQAO.or.Vartyp.eq.PRSQLS).and.(.not.Userin))THEN
...
c     ------------------------------------------------------------------
           IF(begotl.eq.bgotl2)THEN
            IF(otlidx.eq.otlid2)THEN
             WRITE(STDERR,1030)Effttl
             CALL errhdr
             WRITE(Mt2,1030)Effttl
             CALL abend
             RETURN
c     ------------------------------------------------------------------
...
c            IF(rgmidx.lt.rgmid2)GO TO 2
            WRITE(STDERR,1020)Effttl
            CALL errhdr
            WRITE(Mt2,1020)Effttl
            CALL abend
            RETURN
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      IF(Nb.ge.PB)THEN
       ...
       WRITE(STDERR,1010)Effttl,numstr(1:(ipos-1)),LIMSEC,PRGNAM,DOCNAM
       CALL errhdr
       WRITE(Mt2,1010)Effttl,numstr(1:(ipos-1)),LIMSEC,PRGNAM,DOCNAM
 1010  FORMAT(/,
     &        ' ERROR: Adding ',a,' exceeds the number of regression ',
     &        'effects allowed',/,
     &        '        in the model (',a,').',//,
     &        '        Check the regression model, change the ',
     &        'automatic outlier options,',/,
     &        '        (e.g. method to ADDONE, raise the critical ',
     &        'value, or change types',/
     &        '        to identify AOs only), or change the program ',
     &        'limits (see ',a,/,
     &        '        of the ',a,' ',a,').')
       CALL abend
       RETURN
...
c     ------------------------------------------------------------------
      IF(icol.gt.0)THEN
       ...
       CALL errhdr
       WRITE(Mt2,1020)Effttl
 1020  FORMAT(/,' ERROR: ',a,' is already in the regression.')
       CALL abend
       RETURN
      END IF
c-----------------------------------------------------------------------
...
c     ------------------------------------------------------------------
        IF(begotl.eq.bgotl2)THEN
         IF(otlidx.eq.otlid2)THEN
          ...
          CALL errhdr
          WRITE(Mt2,1030)Effttl
 1030     FORMAT(/,' ERROR: ',a,' already exists.')
          CALL abend
          RETURN
c     ------------------------------------------------------------------
         ELSE IF(otlidx.lt.otlid2)THEN
