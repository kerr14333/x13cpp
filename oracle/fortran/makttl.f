...
c-----------------------------------------------------------------------
      IF(Ntbttl.eq.0)THEN
       ...
       IF(Lfatal)RETURN
       CALL writln('ERROR: '//PRGNAM//' could not generate a table '//
     &             'title (internal code #'//ctbl(1:(ipos-1))//').',
     &             STDERR,Mt2,T)
       CALL writln('       Please send the input spec file that '//
     &             'generated this error message,',STDERR,Mt2,F)
       CALL writln('       along with any data files used, to '//
     &             'x12@census.gov.',STDERR,Mt2,F)
       CALL abend
       RETURN
      ELSE
