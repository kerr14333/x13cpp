...
c-----------------------------------------------------------------------
       IF(otltyp.eq.AO.or.otltyp.eq.MV)THEN
        IF(begotl.gt.Iymx+Nspobs.or.begotl.lt.Iymx+1)THEN
         CALL getstr(Colttl,Colptr,Ncoltl,icol,str,nchr)
         IF(Lfatal)RETURN
         IF(.not.(Lquiet.or.Lhiddn))WRITE(STDERR,1020)str(1:nchr)
         CALL errhdr
         WRITE(Mt2,1020)str(1:nchr)
 1020    FORMAT(/,' NOTE: Removing ',a,
       ...
       ELSE IF(otltyp.eq.LS)THEN
        IF(begotl.gt.Iymx+Nspobs.or.begotl.lt.Iymx+2)THEN
         ...
         IF(begotl.eq.Iymx+Nspobs+1)THEN
          IF(.not.(Lquiet.or.Lhiddn))WRITE(STDERR,1030)str(1:nchr)
          CALL errhdr
          WRITE(Mt2,1030)str(1:nchr)
 1030     FORMAT(/,' NOTE: Removing ',a,
     &             ' from the regression because it occurs at the',/,
     &             '       last data point of the span of the data.',/)
         ELSE IF(begotl.eq.Iymx+1)THEN
          IF(.not.(Lquiet.or.Lhiddn))WRITE(STDERR,1040)str(1:nchr)
          CALL errhdr
          WRITE(Mt2,1040)str(1:nchr)
 1040     FORMAT(/,' NOTE: Removing ',a,
     &             ' from the regression because it occurs at the',/,
...
c-----------------------------------------------------------------------
        CALL getstr(Colttl,Colptr,Ncoltl,icol,str,nchr)
        IF(Lfatal)RETURN
        WRITE(STDERR,1050)str(1:nchr)
        CALL errhdr
        WRITE(Mt2,1050)str(1:nchr)
 1050   FORMAT(/,' ERROR: ',a,
