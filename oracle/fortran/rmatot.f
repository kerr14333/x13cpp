      ...
      IF(Lprt.or.Lsav)THEN
       ...
       IF(natrtl.gt.0)THEN
        DO i=1,natrtl
         ...
         END IF
         IF(Lprt)THEN
          IF((nchr-n0+36).gt.limchr)THEN
           n1=nchr-n2-1
           IF(n0.eq.1)THEN
            WRITE(Mt1,1030)datstr(1:nchdat),'kept',outstr(n0:n1)
           ELSE
            WRITE(Mt1,1030)' ',' ',outstr(n0:n1)
           END IF
           n0=n1+1
          END IF
         END IF
       ...
       IF(Lprt.and.nchr.gt.0)THEN
        IF(n0.eq.1)THEN
         WRITE(Mt1,1030)datstr(1:nchdat),'kept',outstr(1:nchr)
        ELSE
         WRITE(Mt1,1030)' ',' ',outstr(n0:nchr)
        END IF
       END IF
       IF(Lsav)THEN
        IF(nchr.eq.0)THEN
        ...
        DO i=1,nrmrtl
         ...
         IF(Lprt)THEN
          IF((nchr-n0+36).gt.limchr)THEN
           n1=nchr-n2-1
           IF(n0.eq.1)THEN
            WRITE(Mt1,1030)datstr(1:nchdat),'deleted(auto)',
     &                     outstr(n0:n1)
           ELSE
            WRITE(Mt1,1030)' ',' ',outstr(n0:n1)
           END IF
       ...
       IF(Lprt.and.nchr.gt.0)THEN
        IF(n0.eq.1)THEN
         WRITE(Mt1,1030)datstr(1:nchdat),'deleted(auto)',outstr(1:nchr)
        ELSE
         WRITE(Mt1,1030)' ',' ',outstr(n0:nchr)
        END IF
       END IF
