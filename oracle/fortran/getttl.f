C     Last change:  BCM  23 Jul 1998    3:38 pm
      SUBROUTINE getttl(Grpchr,Flgnul,Pelt,Chrvec,Ptrvec,Nelt,Locok,
     &                  Inptok)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      ...
      INTEGER clsgtp,clsgrp,Grpchr,ipos,Nelt,Pelt,Ptrvec,nstr1
...
c-----------------------------------------------------------------------
     ...
     &                 'list of either, not an empty string.')
        Locok=F
       ELSE
     ...
     &               str1(1:nstr1-1)//' characters.')
         CALL putstr(Nxttok(1:len(Chrvec)),Pelt,Chrvec,Ptrvec,Nelt)
        ELSE 
...
c     ------------------------------------------------------------------
     ...
     &         'Expected a NAME or a QUOTE or a list of either, not "'//
     &             Nxttok(1:Nxtkln)//'"')
       Locok=F
       opngrp=F
       CALL lex()
...
c     ------------------------------------------------------------------
           ELSE IF(Nelt.ge.Pelt)THEN
            ...
            ipos=ipos+33
            CALL inpter(PERROR,Lstpos,str(1:ipos-1))
            Locok=F
c     ------------------------------------------------------------------
           ELSE
