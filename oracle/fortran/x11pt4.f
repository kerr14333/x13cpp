...
c     ------------------------------------------------------------------
      ...
      IF(issame(Stome,Pos1ob,Posfob))THEN
       CALL writln('NOTE: Seasonal adjustment diagnostics cannot be gene
     &rated because ',fhnote,Mt2,T)
       CALL writln('      the series listed below have either a variance
     & of zero or a ',fhnote,Mt2,F)
       CALL writln('      variance could not be computed:',fhnote,Mt2,F)
       CALL writln('      - the original series adjusted for extreme val
     &ues.',fhnote,Mt2,F)
       lsame=T
      END IF
     ...
     &   dpeq(Vi,DNOTST))THEN
       IF(.not.lsame)THEN
        CALL writln('NOTE: Seasonal adjustment diagnostics cannot be gen
     &erated because ',fhnote,Mt2,T)
        CALL writln('      the series listed below have either a varianc
     &e of zero or a ',fhnote,Mt2,F)
        CALL writln('      variance could not be computed:',fhnote,Mt2,
     &              F)
        lsame=T
       END IF
       CALL writln('      - the irregular component.',STDERR,Mt2,F)
      END IF
c     ------------------------------------------------------------------
      IF(allgud)THEN
...
c     ------------------------------------------------------------------
     ...
     &              F)
        lsame=T
       END IF
       CALL writln('      - the trend component.',STDERR,Mt2,F)
      END IF
c-----------------------------------------------------------------------
...
c     ------------------------------------------------------------------
     ...
     &              F)
       ...
       IF((Adjls.eq.1.or.Adjao.eq.1.or.Adjtc.eq.1).and.Adjusr.eq.1)THEN
        CALL writln('      - the original series adjusted for regARIMA o
     &utlier and user-defined',fhnote,Mt2,F)
        CALL writln('        regression effects.',STDERR,Mt2,F)
       ELSE IF(Adjls.eq.1.or.Adjao.eq.1.or.Adjtc.eq.1)THEN
        CALL writln('      - the original series adjusted for regARIMA o
     &utlier effects.',STDERR,Mt2,F)
       ELSE IF(Adjusr.eq.1)THEN
        CALL writln('      - the original series adjusted for user-defin
     &ed regression',STDERR,Mt2,F)
        CALL writln('        effects.',STDERR,Mt2,F)
       ELSE
        CALL writln('      - the original series.',STDERR,Mt2,F)
       END IF
      END IF
...
c     ------------------------------------------------------------------
     ...
     &              F)
        lsame=T
       END IF
       CALL writln('      - the original series with the linear trend re
     &moved.',fhnote,Mt2,F)
      END IF
      IF(lsame)RETURN
      Vp=Vp/vo
...
c-----------------------------------------------------------------------
     ...
     &    CALL fgen(Mt1,Kfmt,Prttab(LCMPF2),Prttab(LCMPF3),.False.)
       CALL svf2f3(Nform,Ng,Savtab(LCMPF2),Savtab(LCMPF3),'if')
       IF(Lfatal)RETURN
      ELSE
       IF(Prttab(LX11F2).or.Prttab(LX11F3))
     &    CALL fgen(Mt1,Kfmt,Prttab(LX11F2),Prttab(LX11F3),.True.)
