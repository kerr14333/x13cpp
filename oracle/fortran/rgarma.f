...
c-----------------------------------------------------------------------
      LOGICAL intflt,Lestim,lnxstp,locest,Lprtit,stpitr,Lauto,la,gudrun
      INTEGER flterr,i,iflag,info,inverr,iter,lstnit,Mxiter,Mxnlit,Na,
     &        Nefobs,neltxy,nprint,nrtxy,tnlitr
c      INTEGER bset
...
c-----------------------------------------------------------------------
      IF(Nspobs*Ncxy.gt.PXY)THEN
       CALL errhdr
       WRITE(STDERR,1010)Nspobs,Ncxy,PXY
       WRITE(Mt2,1010)Nspobs,Ncxy,PXY
...
*      IF(Nefobs.le.Nextvl)THEN
      IF(Nefobs.lt.Nextvl)THEN
       WRITE(STDERR,1020)Nefobs,Nextvl
       WRITE(Mt2,1020)Nefobs,Nextvl
 1020  FORMAT(/,' ERROR: Number of observations after differencing ',
     &          'and/or conditional AR',
     &        /,'        estimation is',i4,', which is less than the ',
     &          'minimum series length',
     &        /,'        required for the model estimated,',i4,'.',/)
       IF(Lauto)THEN
        CALL writln('        Check automatic modeling options (reduce ma
     &xorder for automdl, or',Mt1,Mt2,T)
        CALL writln('        check models used with pickmdl) and try aga
     &in.',Mt1,Mt2,F)
        Lauto=F
       ELSE
        CALL abend
...
c-----------------------------------------------------------------------
       IF(Nlwrk.gt.PXA)THEN
        WRITE(STDERR,1030)Nlwrk,PXA
        WRITE(Mt2,1030)Nlwrk,PXA
 1030   FORMAT(/,' ERROR: Non linear work array too small',i6,'>',i6,
     &           '.')
        IF(Lauto)THEN
         Lauto=F
        ELSE
...
c-----------------------------------------------------------------------
c      write(ng,9000)'init',iter, lnxstp, info, convrg, armaer
      DO WHILE (T)
...
c-----------------------------------------------------------------------
     ...
     &               Armaer,Lhiddn)
       iter=iter+1
c       write(ng,9000)'lnxstp,iter,convrg',iter, lnxstp, info, convrg,
c     &               armaer
       IF(Lprtit)THEN
        dvec(1)=objfcn
...
c-----------------------------------------------------------------------
     ...
     &             Nliter,Nfev,Armacm,PA,ipvt,qtf,wa1,wa2,wa3,wa4)
        IF(Lfatal)RETURN
c        write(ng,9000)'after lmdif',iter, lnxstp, info, convrg, armaer
        locest=Nb.gt.0
...
c-----------------------------------------------------------------------
     ...
     &          (info.ge.6.and.info.le.8)
        ...
        END IF
c        write(ng,9000)'convrg=',iter, lnxstp, info, convrg, armaer
c-----------------------------------------------------------------------
        IF(info.lt.0)THEN
         Armaer=PUNKER
         Convrg=F
...
c-----------------------------------------------------------------------
        ELSE
         Armaer=info
        END IF
c        write(ng,9000)'armaer=',iter, lnxstp, info, convrg, armaer
       END IF
c-----------------------------------------------------------------------
...
c        CALL fcnar(Na,tmpa,estprm,tmpa,Lauto,info,F)
        ...
        IF(Lfatal)RETURN
c        WRITE(Mtprof,*)' info after fcnar = ',info 
c        write(ng,9000)'after fcnar',iter, lnxstp, info, convrg, armaer
        iflag=2
...
c         write(ng,9000)'after covar',iter, lnxstp, info, convrg, armaer
         Lcalcm=info.eq.0
         IF(.not.Lcalcm)Armaer=PACSER
        ELSE
         Lcalcm=F
        END IF
c        write(ng,9000)'lcalcm=',iter, lnxstp, info, convrg, armaer
       END IF
c-----------------------------------------------------------------------
