...
c-----------------------------------------------------------------------
     ...
     &          BIG=10D16)
c-----------------------------------------------------------------------
      CHARACTER tblttl*(PTTLEN),tyrly*(5),fbase*(110),fobs*(5),fsum*(5),
     &          tfmt*(110),tfmt2*(110),e4lab*(21),dash*(60)
      LOGICAL lp,ltbl
...
c-----------------------------------------------------------------------
      If(Ib.eq.0.and.Ie.eq.0)THEN
       lp=Lpage
       IF(Kpart.eq.4.and.Ktabl.eq.9)lp=F
       IF(lp)THEN
        WRITE(Mt1,Ttlfmt)Newpg,Title(1:Ntitle),Kpage,Serno(1:Nser)
        Kpage=Kpage+1
       END IF
       IF(ntbttl.gt.0)WRITE(Mt1,1020)tblttl(1:ntbttl)
 1020  FORMAT(/,' ',a)
       RETURN
      END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
       CALL tblhdr(Ktabl,Itype,Ixreg,nobs,begtbl,1,Y,tblttl(1:ntbttl))
       IF(Lfatal)RETURN
       wid=Tblwid
       IF(wid.lt.12)wid=12
       write(fobs,1030)wid
 1030  FORMAT('a',i2)
       ...
       WRITE(Mt1,tfmt(1:npos))' Year',(e4lab(i)((21-wid+1):21),i=1,2)
       WRITE(Mt1,tfmt(1:npos))'     ',(e4lab(i)((21-wid+1):21),i=3,4)
       WRITE(Mt1,1010)dash(1:ndash)
 1010  FORMAT(a)
       jyr=begtbl(YR)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
 ...
 1040  FORMAT('f',i2,'.',i1)
       ...
       DO i=1,Ie
        WRITE(Mt1,tfmt(1:npos))jyr,X(i),Y(i)
        IF((.not.Lcmpaq).or.((mod(jyr,nmod)+1).eq.nmod))WRITE(Mt1,1050)
 1050   FORMAT(' ')
        jyr=jyr+1
       END DO
       RETURN
...
c     print out header for table
c-----------------------------------------------------------------------
      CALL tblhdr(Ktabl,Itype,Ixreg,nobs,begtbl,Ny,Y,tblttl(1:ntbttl))
      IF(Lfatal)RETURN
c-----------------------------------------------------------------------
C --- WRITE COLUMN HEADINGS.
c-----------------------------------------------------------------------
      l=Ny+1
      CALL prtcol(l,0,Tblcol,Tblwid,Ny,Mt1,nopp,tyrly(nopp),Disp2,Disp3,
     &            Fmtcol,Colhdr)
      IF(nopp.eq.5)l=l-1
c-----------------------------------------------------------------------
C --- WRITE TABLE.
...
c-----------------------------------------------------------------------
       IF(ldec.eq.Kdec)then
        ...
        npos2=Nfmt2
       ELSE
        write(fobs,1040)Tblwid,ldec
        write(fsum,1040)Tblwid+2,ldec
        fbase=' '
        CALL getstr(TFMDIC,tfmptr,PTFM,Iptr,fbase,ipos)
        IF(Lfatal)RETURN
        CALL cnvfmt(fbase,tfmt,fobs,fsum,ipos,npos)
        fbase=' '
        CALL getstr(TFMDIC,tfmptr,PTFM,Iptr+1,fbase,ipos)
        IF(Lfatal)RETURN
        CALL cnvfmt(fbase,tfmt2,fobs,fsum,ipos,npos2)
       END IF
c-----------------------------------------------------------------------
c     Write out this year's data.
c-----------------------------------------------------------------------
c02  111 WRITE(MT1,IF1) JYR,(TMP(I),I = 1,L)
       CALL wrttbl(tmp,jyr,'XXXXX',l,ldec,Mt1,tfmt(1:npos),Tblwid,
     &             Tblcol,Disp1,Disp2,Disp3,nb,nb2,ipow,l.eq.sp1)
       IF(Lfatal)RETURN
     ...
     &          ((Kpart.eq.2.or.Kpart.eq.3).and.Ktabl.eq.17))THEN
         IF(Ksdev.gt.0)THEN
          DO i=1,Ny
           tmp(i)=Stdper(i)
          END DO
          CALL wrttbl(tmp,0,tyrly(3),Ny,ldec,Mt1,tfmt2(1:npos2),Tblwid,
     &                Tblcol,Disp1,Disp2,Disp3,0,0,ipow,F)
          IF(Lfatal)RETURN
         END IF
c-----------------------------------------------------------------------
...
c02      WRITE(MT1,IF2) TYRLY(NOP1),(TMP(I),I = 1,NY)
     ...
     &               Tblwid,Tblcol,Disp1,Disp2,Disp3,nb,nb2,ipow,F)
         IF(Lfatal)RETURN
...
c-----------------------------------------------------------------------
       CALL prtshd(tblttl(1:ntbttl),begfct,Ny,nfct,lp)
       IF(Lfatal)RETURN
...
c-----------------------------------------------------------------------
       CALL prtcol(l,0,Tblcol,Tblwid,Ny,Mt1,nopp,tyrly(nopp),Disp2,
     &             Disp3,Fmtcol,Colhdr)
c-----------------------------------------------------------------------
c     Print forecasts
c-----------------------------------------------------------------------
