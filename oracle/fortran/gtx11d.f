...
c-----------------------------------------------------------------------
      CHARACTER File*(*),Srsnam*(*),fmtdat*(PFILCR),lab*8
...
c-----------------------------------------------------------------------
     ...
     &       year,lab(1:n)
       ...
       IF(lab(1:n).eq.Srsnam(1:n))THEN
        ...
        ELSE
         ...
         IF(year.ne.nyy)THEN
          ...
          WRITE(Mt2,1150)nyy,Srsnam(1:n),year
          Argok=F
          Nobs=0
          RETURN
         END IF
       ...
       END IF
      END DO
   10 WRITE(STDERR,1130)File
      WRITE(Mt2,1130)File
      Argok=F
      Nobs=0
      RETURN
   20 IF(.not.havsrs)THEN
       WRITE(STDERR,1140)Srsnam(1:n),File
       WRITE(Mt2,1140)Srsnam(1:n),File
       Argok=F
       Nobs=0
      ELSE
...
c-----------------------------------------------------------------------
 ...
 1130 FORMAT(/,' ERROR: Problem reading ',a,'.',/,
     &         '        Check input file and format.',/)
 1140 FORMAT(/,' ERROR: Cannot find series ',a,' in file ',a,'.',/,
     &      '        Check series name, input file, and format.',/)
