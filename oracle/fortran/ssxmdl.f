...
c-----------------------------------------------------------------------
      ...
      IF(nbeg.gt.0.or.nend.gt.0)THEN
       ...
       IF(Itd.gt.0)Itd=-1
       IF(Ihol.gt.0)Ihol=-1
       CALL writln('NOTE: Since a span is used in the x11regression spec 
     &, the irregular ',fhnote,Mt2,T)
       CALL writln('      regression coefficient estimates will be held  
     &fixed during the ',STDERR,Mt2,F)
       CALL writln('      sliding spans analysis.',fhnote,Mt2,F)
      END IF
c-----------------------------------------------------------------------
