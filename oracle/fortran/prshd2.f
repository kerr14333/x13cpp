...
**==prtshd.f    processed by SPAG 4.03F  at 09:52 on  1 Mar 1994
      SUBROUTINE prshd2(Ttlhdr,Subhdr,Begdat,Sp,Nobs,Locpag)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      LOGICAL Locpag
      CHARACTER Ttlhdr*(*),Subhdr*(*),bdtstr*(10),blnk*(PSRSCR),
     &          edtstr*(10)
...
c     ------------------------------------------------------------------
      ...
      IF(Lfatal)RETURN
      IF(len(Ttlhdr).gt.0)THEN
       WRITE(Mt1,1020)' '
       WRITE(Mt1,1020)Ttlhdr
       IF(len(Subhdr).gt.0)WRITE(Mt1,1020)Subhdr
      END IF
 1020 FORMAT(' ',a)
      IF(Nobs.gt.0)THEN
