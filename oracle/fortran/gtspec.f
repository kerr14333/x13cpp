...
c-----------------------------------------------------------------------
   40   CALL gtdcvc(LPAREN,T,1,SPSDIC,spsptr,PSPS,
     ...
     &              ivec,nelt,argok,Inptok)
        IF(Lfatal)RETURN
        IF(nelt.eq.0)THEN
         CALL writln('        Valid entries for spectrumseries are a1, a
     &19, b1, e1, original,',STDERR,Mt2,F)
         CALL writln('        outlieradjoriginal, adjoriginal, or modori
     &ginal.',STDERR,Mt2,F)
...
c-----------------------------------------------------------------------
   50   CALL getivc(LPAREN,T,1,ivec,nelt,argok,locok)
     ...
     &  'Max order for AR spectrum must be between 1 and 30, inclusive')
          locok=F
         ELSE
          Mxarsp=ivec(1)
         END IF
   ...
   60   CALL gtdpvc(LPAREN,T,1,dvec,nelt,argok,Inptok)
        ...
        IF(argok)THEN
         IF(dvec(1).le.ZERO)THEN
          CALL inpter(PERROR,Errpos,
     &         'Visual signifcance criteria must be greater than zero.')
          locok=F
         ELSE
          Spclim=dvec(1)
         END IF
   ...
   70   CALL getivc(LPAREN,T,1,ivec,nelt,argok,locok)
        ...
        IF(argok)THEN
         IF(ivec(1).lt.1.or.ivec(1).gt.4)THEN
          CALL inpter(PERROR,Errpos,
     &        'Spectral peak width must be between 1 and 4, inclusive')
          locok=F
         ELSE
          Peakwd=ivec(1)
         END IF
...
c-----------------------------------------------------------------------
  130   CALL gtdcvc(LPAREN,T,1,AXSDIC,axsptr,PAXS,
     &       'Available options for axis are same, diff or difference.',
     &              ivec,nelt,argok,Inptok)
        IF(Lfatal)RETURN
  ...
  160   CALL gtdpvc(LPAREN,T,1,dvec,nelt,argok,Inptok)
        ...
        IF(argok)THEN
         IF(dvec(1).lt.ZERO)THEN
          CALL inpter(PERROR,Errpos,
     &         'Localpeak must be greater than or equal to zero.')
          locok=F
         ELSE
          Plocal=dvec(1)
         END IF
...
c-----------------------------------------------------------------------
        IF(.not.isdate(Bgspec,Sp))THEN
         CALL inpter(PERRNP,Pos,'Spectrum starting date not valid')
         Havesp=F
         locok=F
        ELSE
         CALL dfdate(Bgspec,Begspn,Sp,nspec)
     ...
     &     'Starting date of spectral plots is before start of series.')
          locok=F
         END IF
         CALL dfdate(Bgspec,Endspn,Sp,nspec)
         IF(nspec.ge.0)THEN
          CALL inpter(PERRNP,Errpos,
     &        'Starting date of spectral plots is after end of series.')
          locok=F
         END IF
        END IF
       END IF
