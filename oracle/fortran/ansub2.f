...
C PRINTING OF THE RESULTS OF RPQ
 7001 format (
     $7x,' REAL PART   ',' IMAGINARY PART','     MODULUS   ',
     $'     ARGUMENT ','    PERIOD')
      write (Nio,7001)
       do i = 1,nroots
        if (ar(i) .ge. 0.0d0) then
         if (ABS(p(i)-999.99) .lt. 1.0d-12) then
 7002     format (6x,f11.4,4x,f11.4,5x,f11.4,4x,f11.4,5x,a4)
          write (Nio,7002) rez(i), imz(i), m(i), ar(i), per(i)
         else
 7003     format (6x,f11.4,4x,f11.4,5x,f11.4,4x,f11.4,1x,f11.4)
          write (Nio,7003) rez(i), imz(i), m(i), ar(i), p(i)
         end if
        end if
       end do
      end
...
c
      do i=1,3
        ...
        Imz(i)=0.0D0
      enddo
      if ((n.ne.4) .or.(P(1).eq.0.0D0)) then
c         ERROR this is not a 3th order polynomial
          return
      end if
      call cubicRoot(p(4)/p(1),0.0D0,Rez(1),Imz(1))
     ...
     $    (Abs(p(2)/p(1)+n3*Rez(1)).lt.eps)) then     
      ...
      b_3a=p(2)/(n3*p(1))
      call DivCompl(Q,0.0D0,p(1)*rS1,p(1)*iS1,rQ_aS,iQ_aS)
      rS_a=rS1/p(1)
      iS_a=iS1/p(1)
...
*     &          ' l1 = ',l1
      ...
      do i = 1,l1
       c(i) = (c1(i)+c2(i)) / 2
C     IF (DABS(C(I)) .LE. 1.0D-10) C(I) = 0.0D0
      end do
      end
C
C
...
c
      if (iX.eq.0.0D0) then
        ...
        w=arg(rX,iX)
        m=rX*rX+iX*iX
        rY=(m**(1.0D0/6.0D0))*cos(w/3.0D0)
        iY=(m**(1.0D0/6.0D0))*sin(w/3.0D0)
      end if
      end
c
...
c
      if (rX .gt. 0.0D0) then
        arg=atan(iX/rX)
      else if (rX.lt.0.0D0) then
        arg=pi-abs(atan(iX/rX))
      else if (iX.gt.0.0D0) then
        arg=pi/2
      else
        arg=-pi/2
      end if
...
C       SUM=DABS(ZZ)
C       DO 678 I=2,NZ
C       IF (ZZ.GT.0.0D0) THEN
C       SUM=0.0D0
C       GOTO 679
C       end if
C       ZZ=DLOG10(DABS(DATA(I)+.0000000001D0))
C       IF ((DABS(ZZ).LT.SUM).AND.(ZZ.LT.0.0D0)) SUM=DABS(ZZ)
C 678  CONTINUE
C 679  IF (SUM.GT.1.0D0) THEN
C        IFACT=IDINT(SUM)
...
C       DO 878 I=2,NZ
C       ZZ=DLOG10(DABS(DATA(I)+.0000000001D0))
C       IF ((ZZ.GT.SUM).AND.(ZZ.GT.0.0D0)) SUM=ZZ
C 878  CONTINUE
