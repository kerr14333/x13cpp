       ...
       IF(iprd2.le.Nusrad)THEN
        IF(Percnt.lt.2)THEN
         IF(Usradj(iprd2).le.ZERO)THEN
          IF(Percnt.eq.0)THEN
           CALL writln('ERROR: Prior adjustment factors expressed as per
     &centages cannot have values',STDERR,Mt2,T)
          ELSE
           CALL writln('ERROR: Prior adjustment factors expressed as rat
     &ios cannot have values',STDERR,Mt2,T)
          END IF
          CALL writln('       less than or equal to zero.',STDERR,Mt2,F)
