...
c-----------------------------------------------------------------------
     ...
     &      )THEN
          ...
          Keastr=0
         END IF
         IF(Ieast(1).eq.0)WRITE(Mt2,1040)i
         IF(Ieast(2).eq.0)WRITE(Mt2,1050)i
         IF(Ieast(3).eq.0)WRITE(Mt2,1060)i
         IF(Ieast(4).eq.0)WRITE(Mt2,1070)i
        END DO
        IF(Keastr.eq.0)THEN
         WRITE(Mt2,1080)
         WRITE(STDERR,1080)
...
*       lidotl=Ltstao.or.Ltstls.or.Ltsttc.or.Ltstso
     ...
     &      Rgvrtp(begcol).eq.PRGTQD.or.Rgvrtp(begcol).eq.PRGTQI.or.     
     &      Rgvrtp(begcol).eq.PRGTSO.or.Rgvrtp(begcol).eq.PRGTRP.or.
     &      Rgvrtp(begcol).eq.PRGTTL)THEN
          ircol=endcol
          DO WHILE (ircol.ge.begcol)
           CALL getstr(Colttl,Colptr,Ncoltl,ircol,str,nchr)
           IF(Lfatal)RETURN
           ...
           IF(.not.locok)THEN
            ...
            RETURN
           END IF
           IF(((otltyp.eq.RP.or.otltyp.eq.TLS).and.(begotl.ge.obeg.or.
     &          endotl.le.oend))
     &    .or.((otltyp.ne.RP.and.otltyp.ne.TLS).and.(begotl.lt.obeg.or.
     &          begotl.gt.oend)))THEN
            CALL dlrgef(ircol,Nrxy,1)
            IF(Lfatal)RETURN
           END IF
...
c-----------------------------------------------------------------------
     ...
     &             Otlfix.or.Ssinit.eq.1)
      END IF
c-----------------------------------------------------------------------
 1030 FORMAT(/,5x,'Easter adjustment cannot be performed during the ',
     &       'sliding spans analysis',a)
 1040 FORMAT(/,5x,'No years with Easter before April 1st in span ',i1,
     &       '.')
 1050 FORMAT(/,5x,'No years with Easter after April 16th in span ',i1,
     &       '.')
 1060 FORMAT(/,5x,'No years with Easter between April 2nd and April ',
     &       '8th in span ',i1,'.')
 1070 FORMAT(/,5x,'No years with Easter between April 8th and April ',
     &       '15th in span ',i1,'.')
 1080 FORMAT(/,5x,'Either choose a longer span for the sliding spans ',
     &       'analysis or',/,5x,'preadjust the series using Easter ',
     &       'effects estimated from a',/,5x,'regARIMA model.')
c-----------------------------------------------------------------------
      RETURN
      END
