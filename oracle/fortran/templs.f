...
c     mxtlcr = max (18, (7 * mxchr) - 1)               ! for fixed width
       WRITE(fmt,1010)mxtlcr+2,mxtlcr+11
 1010  FORMAT('(/,'' '',a,/,''  '',a,t',i2,
     &       ',a,'' '',a,                        /,''  '',',i3,
     &       '(''-''))')
       WRITE(Mt1,fmt)'Tests for Cancellation of Level Shifts',
     &              'Dates of LS Sets','Span','t-value'
       WRITE(fmt,1020)mxtlcr+1
 1020  FORMAT('(2x,a,t',i2,',i5,f8.2)')
      END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
        IF(Prttls)
     &     WRITE(Mt1,fmt)tstttl(1:ipos-2),lstp(endls)-lstp(begls),tstat
        IF(Ldiag)THEN
         nlsrun=nlsrun+1
