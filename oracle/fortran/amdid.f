...
c-----------------------------------------------------------------------
      IF(Prttab(LAUMDL))WRITE(Mt1,1010)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &                bstrma,bstsar,bstsdf,bstsma,bstbic)
      ...
      ELSE
       ...
       WRITE(Mt1,1000)'nonseasonal'
       WRITE(Mt2,1000)'nonseasonal'
       CALL abend
       RETURN
      END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
c     check for more parsimonious, more balanced models
c-----------------------------------------------------------------------
c       IF (((irr2.LT.irr1.OR.iss2.LT.iss1).AND.
c     &       ir1.EQ.ir2.AND.is1.EQ.is2.AND.dbic.LE.vc11.and.Lbalmd)
c     & .OR.(irr2.LT.irr1.AND.ir2.LE.ir1.AND.is2.EQ.is1.AND.
c     &      bstrar(i).GT.0.AND.bstrma(i).GT.0.AND.dbic.LE.vc2
c     &      .and.Lbalmd)
c     & .OR.(((irr2.EQ.0.AND.irr2.LT.irr1.AND.Irdf.GT.0).OR.
c     &       (iss2.EQ.0.AND.iss2.LT.iss1.AND.Isdf.GT.0)).AND.
c     &        ir1.EQ.ir2.AND.is1.EQ.is2.AND.dbic.LE.vc11.and.Lbalmd)
c     & .OR.(irr2.EQ.0.AND.iss2.EQ.0.AND.dbic.LT.vc2.and.Lbalmd)
c     & .OR.(ir2.GT.ir1.AND.irr2.EQ.0.AND.is2.EQ.is1.AND.dbic.LT.vc2
c     &      .and.Lbalmd)
c     & .OR.(is2.GT.is1.AND.iss2.EQ.0.AND.ir2.EQ.ir1.AND.dbic.LT.vc2
c     &      .and.Lbalmd)
c     & .OR.(is2.LT.is1.AND.is2.GT.0.AND.ir2.EQ.ir1.AND.iss2.EQ.0.AND.
c     &      dbic.LE.vc2.and.Lbalmd)
c     & .OR.(i.EQ.2.AND.ir1.EQ.0.AND.ir2.EQ.1.AND.is2.EQ.is1.AND.
c     &      dbic.LT.vc2)
c     & .OR.(ir2.LT.ir1.AND.ir2.GT.0.AND.is2.EQ.is1.AND.dbic.LT.vc2)
c     & .OR.(is2.LT.is1.AND.is2.GT.0.AND.ir2.EQ.ir1.AND.dbic.LT.vc2)
c     & .OR.(bstrar(i).LT.Irar.AND.bstrma(i).EQ.Irma.AND.ir2.GT.0
c     &      .AND.is2.EQ.is1.AND.dbic.LT.vc22))THEN
       ichk=0
 ...
 2000   FORMAT(' ichk (best model vrs. model ',i1,') = ',i3)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      IF(.not.Convrg)THEN
       WRITE(Mt1,1080)Tol
       WRITE(Mt2,1080)Tol
       WRITE(Mt1,1090)MDLSEC,PRGNAM,DOCNAM
       WRITE(Mt2,1090)MDLSEC,PRGNAM,DOCNAM
       CALL abend()
       RETURN
      END IF
c-----------------------------------------------------------------------
      ...
      IF(Prttab(LAUB5M).or.Svltab(LSLB5M).or.Lsumm.gt.0)THEN
       IF(Prttab(LAUB5M))WRITE(Mt1,1040)
       IF(Svltab(LSLB5M))WRITE(Ng,1040)
       DO i=1,NMOD
        CALL mkmdsn(bstrar(i),bstrdf(i),bstrma(i),bstsar(i),bstsdf(i),
     &              bstsma(i),tmpmdl,ntmp)
        IF(Lfatal)RETURN
        IF(Prttab(LAUB5M))WRITE(Mt1,1050)i,tmpmdl(1:ntmp),bstbic(i)
        IF(Svltab(LSLB5M))WRITE(Ng,1050)i,tmpmdl(1:ntmp),bstbic(i)
        IF(Lsumm.gt.0)THEN
       ...
       END DO
      END IF
      IF(Prttab(LAUMCH))WRITE(Mt1,1030)Bstdsn(1:Nbstds)
c-----------------------------------------------------------------------
      RETURN
c-----------------------------------------------------------------------
 1000 FORMAT(/,' ERROR: Cannot make a choice of ',a,' ARMA order due',
     &         ' to',
     &       /,'        model estimation errors within the automatic ',
     &         'model',/,'        selection procedure.')
 1010 FORMAT(//,'  ARIMA Estimates and Likelihood Values for ARMA ',
     &         'Order Identification',/)
* 1020 FORMAT(/,'  Estimation error found during unit root testing ',
*     &       /,'  procedure while fitting inital regARIMA model to the',
*     &         ' series.')
 1030 FORMAT(/,'  Automatic model choice : ',a)
 1040 FORMAT(//,'  Best Five ARIMA Models',/)
 1050 FORMAT('     Model # ',i2,' : ',a,t40,'(BIC = ',f10.3,')')
 1060 FORMAT('automdl.best5.mdl',i1,': ',a)
 1070 FORMAT('automdl.best5.bic',i1,': ',f10.3)
 1080  FORMAT(/,' ERROR: Estimation failed to converge for ',
     &          'automatically identified model',
     &        /,'        when convergence tolerence reset to ',e13.6)
 1090  FORMAT(/,'        Rerun program trying one of the following:',/,
     &         10x,'(1) Allow more iterations (set a larger value of ',
     &         'maxiter).',
     &       /,10x,'(2) Try a different model or different modeling ',
     &       'options.',/,
     &       /,1x,'See ',a,' of the ',a,' ',a,' for more discussion.')
      END
