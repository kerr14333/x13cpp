...
C     Last change:  BCM  28 Sep 99    3:17 pm
      SUBROUTINE gtxreg(Begsrs,Nobs,Endspn,Havsrs,Havesp,Muladd,Xuserx,
     ...
     &                  Endxrg,Fxprxr,Begxot,Endxot,Havxhl,Havxtd,
     &                  Axrghl,Axrgtd,Lxrneg,Cvxalf,Cvxtyp,Cvxrdc,
     &                  Xraicd,Inptok)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
  100   CALL gtdcvc(LPAREN,F,PUREG,USXDIC,usxptr,PUSX,
     ...
     &              ' of '//DOCNAM//'.',usxidx,Nusxrg,argok,Inptok)
        IF(Lfatal)RETURN
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
        IF(argok.and.nelt.gt.0)THEN
         IF(dvec(1).le.0)THEN
          CALL inpter(PERROR,Errpos,'Trading day sigma limit must be gre
     &ater than zero.')
          Inptok=F
         ELSE
          Sigxrg=dvec(1)
...
c     ------------------------------------------------------------------
        IF(argok.and.nelt.gt.0)THEN
         ...
         ELSE IF(dvec(1).lt.0)THEN
          CALL inpter(PERROR,Errpos,'Critical value for outlier detectio
...
c     usermean argument
c-----------------------------------------------------------------------
  130   IF(hvumfl)CALL inpter(PERROR,Errpos,'Getting mean from a file.')
c     ------------------------------------------------------------------
...
c-----------------------------------------------------------------------
  180   CALL gtdcvc(LPAREN,T,1,MTDDIC,mtdptr,PMTD,
     &              'Choices are ADDONE or ADDALL',ivec,nelt,argok,
     &              Inptok)
        IF(Lfatal)RETURN
c----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
  200   CALL gtdpvc(LPAREN,T,7,Dwt,neltdw,argok,Inptok)
        ...
        IF(argok.and.neltdw.ne.7)THEN
         CALL inpter(PERROR,Errpos,'Must have seven prior trading day we
     &ights.')
         Inptok=F
        END IF
        GO TO 350
...
c-----------------------------------------------------------------------
  210   CALL gtdcvc(LPAREN,T,3,NAPDIC,napptr,PNAP,
     &              'Choices are TD or HOLIDAY.',napidx,nelt,argok,
     &              Inptok)
        IF(Lfatal)RETURN
c----------------------------------------------------------------------
        IF(nelt.gt.0)THEN
...
c-----------------------------------------------------------------------
  250   CALL gtdtvc(Havesp,Sp,LPAREN,F,2,spnxrg,nelt,argok,Inptok)
        IF(Lfatal)RETURN
        IF(nelt.eq.1)THEN
         CALL inpter(PERROR,Errpos,'Need two dates for the irregular com
     &ponent regression span or')
...
c-----------------------------------------------------------------------
  260   CALL gtdtvc(T,Sp,LPAREN,F,2,spnotl,nelt,argok,Inptok)
        ...
        IF(nelt.eq.1)THEN
         CALL inpter(PERROR,Errpos,
     &   'Need two dates for the span or use a comma as a place holder.'
     &   )
         Inptok=F
        ELSE
         hvotsp=T
  ...
  270   CALL getivc(LPAREN,T,1,ivec,nelt,argok,Inptok)
        ...
        IF(argok)THEN
         IF(ivec(1).lt.0.or.ivec(1).gt.5)THEN
          CALL inpter(PERROR,Errpos,
     &    'Number of input decimals must be between 0 and 5, inclusive')
          Inptok=F
         ELSE
          numdec=ivec(1)
         END IF
  ...
  330   CALL gtdpvc(LPAREN,T,1,dvec,nelt,argok,Inptok)
        ...
        IF(argok.and.nelt.gt.0)THEN
         IF(dvec(1).le.ZERO.or.dvec(1).gt.PTONE)THEN
          CALL inpter(PERROR,Errpos,
     &             'Value of criticalalpha must be between 0 and 0.10.')
          Inptok=F
         ELSE
          Cvxalf=dvec(1)
         END IF
  ...
  346   CALL gtdpvc(LPAREN,T,1,dvec,nelt,argok,Inptok)
        ...
        IF(argok.and.nelt.gt.0)THEN
         IF(dvec(1).le.ZERO)THEN
          CALL inpter(PERROR,Errpos,
     &                'Value of almost must be greater than 0.')
          Inptok=F
         ELSE
          Cvxrdc=dvec(1)
         END IF
...
c-----------------------------------------------------------------------
     ...
     &    CALL gtfldt(peltux,xrfile,nflchr,havfmt,xrfmt(1:nfmtch),2,
     &                Xuserx,neltux,Havesp,Sp,F,' ',0,F,' ',0,0,hvstrt,
...
c-----------------------------------------------------------------------
       IF(hvmdsp)THEN
        ...
        IF(.not.chkcvr(Begspn,Nspobs,Begxrg,nxrg,Sp))THEN
         CALL inpter(PERRNP,Errpos,'Irregular component regression span
     &not within the span of available data.')
...
c----------------------------------------------------------------------
       IF(hvotsp)THEN
        ...
        IF(.not.chkcvr(Begsrs,Nobs,Begxot,nelt,Sp))THEN
         CALL inpter(PERROR,Errpos,'Span not within the series')
         CALL cvrerr('Series',Begsrs,Nobs,'outlier test span',Begxot,
     &               nelt,Sp)
         Inptok=F
        ELSE IF(.not.chkcvr(Begxrg,nmdl,Begxot,nelt,Sp))THEN
         CALL inpter(PERROR,Errpos,'Span not within the model span')
...
c     ------------------------------------------------------------------
         ELSE
          idisp=Grp(Ngrp)-1
*          call profiler(2,'adding user defined regressors')
          DO i=1,Ncxusx
           idisp=idisp+1
...
c-----------------------------------------------------------------------
       IF(Inptok.and.(hvumfl.or.Haveum))THEN
        IF(Ncxusx.eq.0)THEN
         WRITE(STDERR,1025)neltux,Ncxusx
         WRITE(Mt2,1025)neltux,Ncxusx
 1025    FORMAT(/,' ERROR: User-defined mean can only be specified if ',
     &          /,'        user defined regressors are also present.',/)
         Inptok=F
        ELSE IF(hvumfl.and..not.Haveum)THEN
         CALL gtfldt(PLEN,umfile,numfch,hvumft,umfmt(1:numftc),ltrim,
     &               Umean,neltum,Havesp,Sp,hvumnm,umname,numnam,hvuttl,
     &               umtit,numtit,numdec,hvumst,Begum,1,Begspn,Endspn,F,
     &               Haveum,Inptok)
        END IF
...
c-----------------------------------------------------------------------
     ...
     &      igrp=strinx(F,Grpttl,Grpptr,1,Ngrptl,'User-defined Holiday')
         IF(igrp.eq.0)THEN
          WRITE(STDERR,1030)
          WRITE(Mt2,1030)
 1030     FORMAT(' ERROR: Cannot specify group types for user-defined ',
     &           'irregular component',/,
     &           '        regression variables if user-defined ',
     &           'irregular component',/,
     &           '        regression variables are not defined in the ',
     &           ' x11regression spec.')
          Inptok=F
         END IF
c----------------------------------------------------------------------
...
c        IF(dpeq(Sigxrg,DNOTST))Sigxrg=2.5D0
        IF(.not.(Axrgtd.or.Axrghl.or.neltdw.gt.0))THEN
         WRITE(STDERR,1040)
         WRITE(Mt2,1040)
 1040    FORMAT(' ERROR: Must adjust for either trading day or ',
     &          'holiday in the x11regression spec.')
         Inptok=F
        END IF
c-----------------------------------------------------------------------
        ...
        IF(Noxfac.and.(Ixrgtd.eq.0.or.Ixrghl.eq.0))THEN
         WRITE(STDERR,1050)
         WRITE(Mt2,1050)
 1050    FORMAT(' ERROR: Cannot specify noapply when user-defined ',
     &          'mean is also present.')
         Inptok=F
        END IF
       END IF
c----------------------------------------------------------------------
