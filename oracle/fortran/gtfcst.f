   ...
   20   CALL getivc(LPAREN,T,1,ivec,nelt,argok,Inptok)
        ...
        IF(nelt.gt.0)THEN
         IF(ivec(1).gt.PFCST)THEN
          CALL inpter(PERROR,Errpos,'Too many forecasts specified')
          Inptok=F
         ELSE
          Nfcst=ivec(1)
         END IF
...
c-----------------------------------------------------------------------
   30   CALL gtdpvc(LPAREN,T,1,dvec,nelt,argok,Inptok)
     ...
     &         'Coverage probability must be strictly between 0 and 1.')
          Inptok=F
         ELSE
          Ciprob=dvec(1)
         END IF
...
c-----------------------------------------------------------------------
   60   CALL getivc(LPAREN,T,1,ivec,nelt,argok,Inptok)
        ...
        IF(Nbcst.gt.PFCST)THEN
         CALL inpter(PERROR,Errpos,'Too many backcasts specified')
         Inptok=F
        END IF
        GO TO 80
c-----------------------------------------------------------------------
