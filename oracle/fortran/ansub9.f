...
C
      subroutine USRENTRY(rbuff,brbuff,erbuff,lim1,lim2,ifunc)
C
C.. Implicits ..
      implicit none
C
...
C.. In/Out Status: Read, Not Written ..
      integer brbuff,lim1,lim2,erbuff
C.. In/Out Status: Not Read, Not Written ..
      real*8 rbuff(lim1:lim2)
C.. In/Out Status: Not Read, Not Written ..
      integer ifunc
...
c-----------------------------------------------------------------------
      ELSE IF (IFUNC.eq.1139) THEN
       allzro = .true.
       DO i = BRBUFF, ERBUFF
        Sttfsa(i-BRBUFF+1) = RBUFF(i)
        allzro = dpeq(RBUFF(i),0D0).and.allzro
       END DO
       Hvftsa=.not.allzro
      ELSE IF (IFUNC.eq.1135) THEN
       allzro = .true.
       DO i = BRBUFF, ERBUFF
        Sttftr(i-BRBUFF+1) = RBUFF(i)
        allzro = dpeq(RBUFF(i),0D0).and.allzro
       END DO
       Hvfttr = .not.allzro
      ELSE IF (IFUNC.eq.1141) THEN
       allzro = .true.
       DO i = BRBUFF, ERBUFF
        Sttfsf(i-BRBUFF+1) = RBUFF(i)
        allzro = dpeq(RBUFF(i),0D0).and.allzro
       END DO
       Hvftsf = .not.allzro
      ELSE IF (IFUNC.eq.1133) THEN
       allzro = .true.
       DO i = BRBUFF, ERBUFF
        Sttfor(i-BRBUFF+1) = RBUFF(i)
        allzro = dpeq(RBUFF(i),0D0).and.allzro
       END DO
       Hvftor = .not.allzro
      ELSE IF (IFUNC.eq.1170) THEN
       allzro = .true.
       DO i = BRBUFF, ERBUFF
        Sttfcy(i-BRBUFF+1) = RBUFF(i)
        allzro = dpeq(RBUFF(i),0D0).and.allzro
       END DO
       Hvftcy = .not.allzro
      ELSE IF (IFUNC.eq.1139) THEN
...
C Modified by REG on 30 Aug 2005 to add l_nfixed to NMLSTS parameter list
      ...
      subroutine NMLSTS(l_Nochmodel,l_type,l_init,l_lam,l_imean,l_p,
     $   l_d,l_q,l_bp,l_bd,l_bq,
     $   l_sqg,l_mq,l_m,l_iqm,l_maxit,l_fh,l_noserie,l_pg,l_modelsumm,
     $   l_out,l_seas,l_noadmiss,l_OutNA,l_stochTD,
     $   l_iter,l_qmax,l_har,l_bias,l_tramo,
     $   l_model,l_noutr,l_nouir,l_nous,l_npatd,l_npareg,l_interp,l_rsa,
     $   l_fortr,l_neast,l_epsiv,l_epsphi,l_ta,l_xl,l_rmod,
     $   l_blqt,l_tmu,l_phi,l_th,l_bphi,l_bth,l_thlim,l_bthlim,l_crmean,
     $   l_hplan,
     $   l_hpcycle,l_rogtable,l_centrregs,
     $   l_statseas,l_units,l_kunits,l_acfe,l_posbphi,l_printphtrf,
     $   l_tabtables,l_psieinic,l_psiefin,
     $   l_firstobs,l_lastobs,l_HPper,l_maxSpect,l_brol,l_blamda,
     $   l_bserie,l_bmid,l_bcMark,l_Odate,l_Olen,l_DetSeas,
     $   l_nds,nz,l_nfixed,icode,ifail)
C
C
C.. Implicits ..
      implicit none
...
C.. Formal Arguments ..
     ...
     $        l_npatd,l_npareg,l_interp,l_rsa,l_fortr,l_neast
      integer l_nous,l_Nochmodel,nz,ifail
...
*            L_OUT = 3
          ...
          ELSE
           ...
           IF(istrue(Prttab,LSETRN,NTBL-11))L_OUT=3
           Lsgud=T
          END IF
          L_TRAMO = 1
...
c   parameter list
     ...
     $       l_maxit,l_fh,l_noserie,l_pg,l_modelsumm,
     $       l_out,l_seas,l_noadmiss,l_outNa,l_stochTD,l_iter,
     $       l_qmax,l_har,l_bias,l_tramo,l_model,l_noutr,
     $       l_nouir,l_nous,l_npatd,l_npareg,l_interp,
     $       l_rsa,l_fortr,l_neast,l_epsiv,
     $       l_epsphi,l_ta,l_xl,l_rmod,
     $       l_blqt,l_tmu,l_phi,l_th,l_bphi,l_bth,
     $       l_thlim,l_bthlim,l_crmean,l_hplan,
     $       l_hpcycle,l_rogtable,l_centrregs,
     $       l_statseas,l_units,l_kunits,
     $       l_acfe,l_posbphi,l_printphtrf,
     $       l_tabtables,l_psieinic,l_psiefin,
     ...
     $       l_Odate,l_Olen,l_DetSeas,l_nds,nz,l_nfixed)
C
C
C.. Implicits ..
      implicit none
