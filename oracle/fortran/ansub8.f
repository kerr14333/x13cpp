...
!DEC$ IF DEFINED (TSW)
cdos
cdos        open(devnum,file=filename(1:flen),status='old',
cdos     $                              position='append')
cunix
...
cc No queremos la fecha en main output devnum= 16,70 y 71
        if (index(filename,dir(1:ISTRLEN(dir))) .ge. 1) then
         if (devnum.ne.16) then
...
*C.. Parameters ..
*      integer kl,mp,kp
*      parameter (kl = PFCST,kp = 65, mp = 600)      
*c Formal parameters  
...
*        if (ifault .eq. 0) then           
*          na=min(nz,nres)
*          write(48,'(I3,/,I2,/f8.3,/,2X,A)')na,1,-0.0d0,TITLEG
