...
c     ------------------------------------------------------------------
       IF(Nxtktp.ne.RPAREN)THEN
        CALL inpter(PERROR,Lstpos,'Expected ")" after (AR DIFF MA')
        Locok=F
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
        IF(Nxtktp.eq.INTGR)THEN
         ...
         argok=getint(facsp)
         IF(facsp.le.0)THEN
          CALL inpter(PERROR,Lstpos,'Period specified in (AR DIFF MA)per
     &iod must be greater than zero.')
          Locok=F
         END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
          ...
          IF(Mxarlg.gt.PORDER)THEN
           CALL inpter(PERROR,arerr,
     &                 'Order of the AR operator is too large.')
           Locok=F
          END IF
...
c     ------------------------------------------------------------------
          IF(Lseadf.and.Lseff)THEN
           CALL inpter(PERROR,dferr,'Cannot have '//
     &        'a seasonal difference with seasonal regression effects.')
           Locok=F
          END IF
c     ------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &          'Order of the full differencing operator is too large.')
         ...
         ELSE IF(ndf.lt.0)THEN
          CALL inpter(PERROR,dferr,'Order of the differencing operator c
     &annot be less than zero.')
          Locok=F
         END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
   ...
   10  CONTINUE
      END DO
      CALL inpter(PERROR,Lstpos,'Expected "(" in  (AR DIFF MA)')
      Locok=F
c     ------------------------------------------------------------------
   20 IF(Locok)THEN
       Nmdl=Nmdl+1
