...
c-----------------------------------------------------------------------
   10   CALL gtdcvc(LPAREN,T,1,MODDIC,modptr,PMODE,
     ...
     &              ivec,nelt,argok,Inptok)
        IF(Lfatal)RETURN
        IF(nelt.le.0)THEN
         CALL writln('       mult, add, logadd or pseudoadd.',STDERR,
     &               Mt2,F)
        ELSE
         Muladd=ivec(1)
         IF(argok)THEN
...
c-----------------------------------------------------------------------
   20   CALL gtdpvc(LPAREN,F,2,sigl,nelt,argok,Inptok)
     ...
     &     'Two sigma limits needed (or use a comma as place holder).')
         Inptok=F
        ELSE IF(nelt.gt.0)THEN
         ...
         IF(sigl(1).le.ZERO.or.sigl(2).le.ZERO)THEN
          CALL inpter(PERROR,Errpos,'Sigma limits must be greater than z
     &ero.')
          Inptok=F
c-----------------------------------------------------------------------
         ELSE IF(sigl(1).gt.sigl(2))THEN
          CALL inpter(PERROR,Errpos,'Lower sigma limit must be less than
     & upper sigma limit.')
          Inptok=F
c-----------------------------------------------------------------------
         ELSE
...
c-----------------------------------------------------------------------
   30   CALL gtdcvc(LPAREN,F,PSP,SFDIC,sfptr,PSF,
     ...
     &              isf,nelt,argok,Inptok)
        IF(Lfatal)RETURN
        IF(nelt.eq.0)THEN
         CALL writln('       Valid choices of seasonal filter are s3x1, 
     &s3x3, s3x5, s3x9,',STDERR,Mt2,F)
...
c-----------------------------------------------------------------------
     ...
     &           cny(1:(i-1))//' seasonal filters (or use a comma as a')
          CALL writln('       place holder).',STDERR,Mt2,F)
          Inptok=F
         END IF
        END IF
        GO TO 330
...
c     ------------------------------------------------------------------
        IF(argok.and.nelt.gt.0)THEN
         IF(mod(ivec(1),2).eq.0.or.ivec(1).le.0)THEN
          CALL inpter(PERROR,Errpos,'Length of Henderson trend filter mu
     &st be a positive odd integer.')
          Inptok=F
         ELSE
          Ktcopt=ivec(1)
         END IF
...
c-----------------------------------------------------------------------
  130   CALL gtdcvc(LPAREN,T,1,BNDDIC,bndptr,PBND,'Available options for
     & calendarsigma are none, signif, all or select.',ivec,nelt,argok,
     &              Inptok)
        IF(Lfatal)RETURN
        Ksdev=ivec(1)
        GO TO 330
...
c-----------------------------------------------------------------------
  140   CALL gtdcvc(LPAREN,T,PSP,SUMDIC,sumptr,PSUM,
     ...
     &              calidx,nelt,argok,Inptok)
        IF(Lfatal)RETURN
        IF(nelt.eq.0)THEN
         CALL writln('       Valid choices for sigmavec are the name(s) 
     &of a month or',STDERR,Mt2,F)
         CALL writln('       quarter.',STDERR,Mt2,F)
        END IF
c-----------------------------------------------------------------------
        IF(argok.and.nelt.gt.0)THEN
         IF(.not.Havesp)THEN
          ...
          DO i=1,nelt
           ...
           ELSE
            IF(Sp.eq.12.and.isvc.ge.25)THEN
             CALL inpter(PERROR,Errpos,'Entry for sigmavec not valid for
     & monthly data.')
             Inptok=F
             isvc=NOTSET
