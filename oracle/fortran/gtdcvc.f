...
c     ------------------------------------------------------------------
      ...
      LOGICAL argok,Flgnul,hvcmma,Inptok,Locok,opngrp
...
c-----------------------------------------------------------------------
      ELSE IF(Nxtktp.eq.NAME.or.Nxtktp.eq.QUOTE)THEN
       ...
       IF(.not.argok.or.argidx.eq.0)THEN
        CALL inpter(PERROR,Lstpos,Errmsg)
        Locok=F
       ELSE
        Nelt=1
        Idxvec(Nelt)=argidx
...
c     ------------------------------------------------------------------
     ...
     &  'Expected a name or a quote or a list of names or quotes, not "'
     &    //Nxttok(1:Nxtkln)//'"')
       Locok=F
       opngrp=F
       CALL lex()
